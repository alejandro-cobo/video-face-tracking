...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        "Video face tracking demo to visualize detections."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Maximum displacement of a bounding box to consider it the same as the previous frame. Default: 0.3 (30 percent of the max side of the box).",
    )
    parser.add_argument(
        "--cos-sim-thresh",
        type=float,
...
def main(argv: list[str]) -> None:
    ...
    if Path(filename).suffix not in VIDEO_FORMATS:
        raise ValueError(
            f"Input file is not a video {filename} ({VIDEO_FORMATS})."
        )
